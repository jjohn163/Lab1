...
public:
	...
	void branchParticle() {
		...
		for (int i = 0; i < limit; i++) {
			...
			float gravity_effect = 5;
			float life_length = rand() % 3 + 2.0f;
			float scale = 0.5f;
			particleSystem->addNewParticle(branch_name, "Feather", particle_pos, random_rotation, velocity, gravity_effect, life_length, scale);
		}
	}
	...
	void render() {
		...
		if (DEBUG) {
			...
			for (shared_ptr<Entity> entity : entities) {
				float difference = bird->position.y - entity->position.y;
				if (!ViewFrustCull(entity->position, entity->cullRadius) && (entity->moving || (difference < 1500 && difference > -100))) {
					entity->draw(Model);
				}
			}
			Model->popMatrix();
